...
% vectorizing the output matrix y
eye_matrix = eye(num_labels);
y_matrix = eye_matrix(y,:);

%frwd propogation parameters
a1 = [ones(m,1) X];
z2 = a1*Theta1';
a2 = sigmoid(z2);
f=size(a2,1);
...
%computing cost function with regularization x=considering bias parameters
regularization = (lambda/(2*m))*((sum(sum(Theta1(:,2:end).^2)))+ (sum(sum(Theta2(:,2:end).^2))));
J = J + regularization;
 

%computng gradient for backpropogation
